package io.alerium.headjoin.util;
...
public final class HeadMessage {
    ...
    public static List<Component> getComponentList(final UUID skull, final int height, final boolean center, final List<String> message) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            if (message != null && message.size() > y) {
                String messageText = message.get(y);
                Component actualMessage = MiniMessage.miniMessage().deserialize(messageText);
                line = line.append(MiniMessage.miniMessage().deserialize(center ? centeredText(PlainTextComponentSerializer.plainText().serialize(actualMessage), messageText, ChatPaginator.AVERAGE_CHAT_PAGE_WIDTH - height) : messageText));
            }
     ...
     */
    private static String centeredText(final String s, final String a, final int length) {
        if (s.length() >= length)
            return a;
